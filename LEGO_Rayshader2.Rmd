...
--->
  ...
  Dirk had a wondelfull idea, to build from scratch a <a href="https://dirks-blog.tumblr.com/LEGO-world-map">world map out of LEGO</a> (<a href="https://www.flickr.com/photos/dirkb86/35962608590/in/album-72157683748271032/">large image</a>). Shout out to **<a href="https://twitter.com/dirksblog#_=_">Dirk**</a>!
  
![](./mapa LEGO 50 X 50.png)
  
  I love LEGO, and I love maps even more!
  
  I became obsessed with this ideia. Sure, who wouldn't?! The problem is the high cost of the endevor... I started looking for a cheaper way from China, but couldn´t find a good source. 
  
  Then my third love come to me and said: look at me! I taking about data visualization, in the generic form of R, and in the specific form of a package called **<a href="https://github.com/tylermorganwall/rayshader">Rayshader</a>**. Shout out to **Tyler Morgan-Wall**!
  
  The ideia is simple, to construct my version of this LEGO map in R. It´s fun, not the same fun, but fun nonetheless.
  
  Than before starting I thought, if I am going to do it, let´s do it in the form of a walkthrough, so other people could use this as a guide to repeat the process, or as an introduction to Rayshader. I tried to be as explicid (or verbose) as I could on what I was doing.
  
  The walkthrough is divided in several parts. First we learn how load and read an *.png* image in order to collect the colors of the map. Then we manipulate the data in order to make a similar map in R. Theardly we use the functions of rayshader extensively, trying to understand how it works. And finally we apply rayshader in our map, and make a complex gif of the first voyage of Columbus to America. 
...
```
  ...
  Now, note that the numbers are different of the patterns RGB that we know (<a href="https://www.w3schools.com/colors/colors_picker.asp">in here</a>). That´s because we have a continuum from 0 to 1 representing the more common 1 to 256. If we simple multiply 256 using our numbers above, we have the result we are looking for.
...
color.center.first.square
```
  Cool. Just to be sure that the colors are exactly the same throughout, let´s visit the second square that we know are suppose to be the same color as the first.
...
![](./Chile.png ){ width=75% }
  ...
  The Alaska bit above starts at square (9, 1), that is, counting from the top left, 9 squares down, 1 square to the right. The South America bit starts at (56, 48). Both of them have width of 20 squares and height of 16 squares. 
...
```
  ...
  Sorry for the cumbersome code and output. For debugging purposes I concatenate (*paste()*) the three colors in one string. My first thought was to use just one of the three as a marker, but than I feared that more than one point would have the same red property. 
...
## Giving the colors a value
  ...
  We arrive in a moment of decision, we could put values similar to the actual world height, as the legend suggests, or we can put LEGO heights, that is discrete and incremental values, so that when rayshader do its work color represent height not height represents heights. Well, that was a confusing and correct sentence! This is a LEGO world, the choice is obvious.
...
```
  ...
  *Ggplot2::geom_tile* organized the order of columns and rows with their names. And V10, not V2 follows V1. 
  ...
  Additionally, for aesthetics let´s define de borders between the tiles as black. 
...
```
  ...
  Let´s deal with the colors. We have to make a scale such that the continuum of numbers (heights) show as a discrete list of colors. Note that we have to stick to a continuum in the numbers, rayshader is going to use the values to make the z axis. We just have to change de colors instead. In other words we keep the scale continuous, but with a discrete feeling.
  
  I had to feedle around a bit to find how to do such a trick. The answer (<a href="https://stackoverflow.com/questions/10981324/ggplot2-heatmap-with-colors-for-ranged-values">here</a>) comes with the functions **scale_fill_gradientn** and <a href="https://www.rdocumentation.org/packages/scales/versions/0.4.1/topics/rescale">**rescale**</a>. Combining the two I can set intervals for my continuous/discrete color scale.
...
```
  ...
  I had to look it up how to manually get rid of most of the margins and other features of ggplot in this graph (*g + theme* above). There are some white tiles in the map. On the center of some of them there is a smaller tile with the color of the altitude (I use this, as I take from each square the color of the central pixel). Still there are some white tiles that don't have this features, so I decided to set white as light green (**world.matrix[world.matrix=="253256252"] <- 3**). 
...
## Shading LEGO
  ...
  Now for the cool part. The function of Rayshader **plot_gg** uses a ggplot object and make it 3d. In our case, a geom_tile, it uses the value used to make the color of the tile to expand it in the z axis. I put the same width and height as the canvas done previuosly, in order to get less white stuff (but it aint perfect yet).
...
```
  ...
  * **clear**	= Default 'FALSE'. If 'TRUE', the current 'rgl' device will be cleared.
  ...
  I am no photographer and I don´t claim to have understand all these features, but I can appreciate the extend of the awesome work done here! Congratz Tyler!

## Let´s move the camera

  Now, with the function **rayshader::render_movie** we can movement the camera position and attributes doing a movie (mp4, gif). We just play with the parameters phi, theta and zoom. We are going to make three lists for each of these parameters. Each list will have 390 point of view (they have to be the same length).
...
## Making a steady-cam gif, with focal change.
  ...
  We did make a photo of the 3d image with focal parameters. As I understood there is no focal parameters in the **render_movie type=custom**, so we are going to stitch together a bunch of still images in a gif. Let´s do another for loop. In this loop I will start at the 60 and go to 95, these are the *focus* (focuses) that will be changing over time. 
...
Due to the limits on gitpages here the 

<p1>**<a href="https://github.com/arthurwelle/RayshaderWalkthrough/blob/master/movie2.gif">LINK TO GIF</a>**.</p1>
...
# Columbus Voyage in a virtual LEGO map.
  ...
  Fist of all, to get another color in the same map, how do we go about it? Add another height? Could be done, but it might be strange. I will interpolate every color/number with a red middle point. So every time we have a 1.5, or a 4.5 in the matrix representing the heights that point will be highlighted in red (with no detriment to the other colors). 
...
```
  ...
  Ok, now it's time to make one still-frame image of the voyage with rayshader. I will feedle around with the parameters to find good points of view. Then I will record they in my drive as .pngs. That´s a big *for loop*: construct a new ggplot with the position of Columbus, take the photo and start over. 
  
  Before the loop, we soon arrive at a problem. The point of view are a globe around the object. Say you want a photo of that object from other angle, like if you take a step to the side? You be out of the globe of points... So the work around is to change the center of the object, either construction more blank space in some of the sides of the object or you can select a subset of the object to be displayed, this time center at the desired point. Let´s try both of the approaches.
...
```{r , include=TRUE, message=FALSE, warning=FALSE, eval=FALSE, echo=TRUE}
#list of images, reversed and concatenated
frames <- paste0("Pic", 1:77, ".png")
#frames <-c(frames, rev(frames))

#do the magick!
m <- magick::image_read(frames)
m <- magick::image_animate(m, fps = 5)

#save the gif
magick::image_write(m, "Columbus4.gif")
```

Due to the limits on gitpages here the 

<p1>**<a href="https://github.com/arthurwelle/RayshaderWalkthrough/blob/master/Columbus3.gif">LINK TO GIF</a>**.</p1>
  
  Yep, that it!
  
### A little map on the top of the gif.

  Let´s improve the gif with a well behave map, and some information on top. First a little map to be put on top of the video/gif. It´s just a copy of previous work, with small changes.

```{r , include=TRUE, message=FALSE, warning=FALSE, eval=FALSE, echo=TRUE, fig.height = 2, fig.width = 2} 
#create a copy of the data, with no missing, and just a part
d1<-na.omit(d)  %>% 
  dplyr::filter(Row %in% c( paste0("R", 125:175) ))%>%
  dplyr::filter(Hight %in% c( paste0("V", 144:195) ))
...
ggplot2::ggsave(filename = paste0("F",i,".png"), plot = g2, width = 2, height = 2, units = "cm" )
...
Text<-c("03 ago 1492", rep("ago 1492",10), rep("sep 1492",22), rep("out 1492",6),"12 out 1492", rep("out 1492",4), rep("nov 1492",2), rep("dec 1492",1), rep("jan 1493",7), rep("feb 1493",20), rep("mar 1493",2), "04 mar 1493")

for (i in 39:41) { 
  
# all magick manipulation of images
little_pic<-magick::image_read(path = paste0("F",i,".png"))

#take out some unstandard border
little_pic<-magick::image_trim(little_pic)
little_pic<-magick::image_crop(little_pic, "224X224")

#add black border, 2 pixels 
little_pic<-magick::image_border(little_pic, "black", "2X2")

#read the rayshader image
large_pic<-magick::image_read(path = paste0("Pic",i,".png"))

#make a colage from the two images
colage<-magick::image_flatten(c(large_pic, little_pic))

# add some text
colage <- magick::image_annotate(colage, text = Text[i], font = "mono", location = "+8+182", boxcolor = "white", size = 30)   
colage <- magick::image_annotate(colage, text = "Columbus Voyage", font = "times", location = "+4+4", size = 30, boxcolor = "white")  
colage <- magick::image_annotate(colage, text = "@ArthurWelle", font = "mono", location = "+4+708", boxcolor = "white", size = 24)  

#save with another name
magick::image_write(image = colage, path = paste0("T",i,".png"))

}

# Add some text in specific images
T1<-magick::image_read(path = "T40.png")
T1 <- magick::image_annotate(T1, text = "Arrive at Guanahani Island", font = "times", location = "+288+8", size = 40, boxcolor = "white") 
magick::image_write(image = T1, path = "T40.png")

```

  Done! And it's time to make the final gif.

```{r , include=TRUE, message=FALSE, warning=FALSE, eval=FALSE, echo=TRUE}
#list of images, reversed and concatenated
frames <- paste0("T", 1:77, ".png")
#frames <-c(frames, rev(frames))
...
#save the gif
magick::image_write(m, "Columbus10.gif")
```

Due to the limits on gitpages here the 

<p1>**<a href="https://github.com/arthurwelle/RayshaderWalkthrough/blob/2e7a274d611fb02357a97e5921ec44c049681946/Columbus11.gif">LINK TO FINAL GIF</a>**.</p1>

  Finish! That´s my take on this issue. Turns out to be a longer take than I expected. 
  
  I hope you enjoy the ride/sail as well. There is of course things that can be done better. One that bothered me a bit was the lack of function using/making. Repeating the code when it´s not necessary is a bad practice, but I keep it that way so it would be clearer for any user what's going on in every part of the code (I could pin point some of code lines that lack clarity! Sorry!). Furthermore I'm still learning R code every day. Don't take my code at face value. 
  ...
  I suppose I must cite here in the end **<a href="https://github.com/ryantimpe/brickr">brickr</a>**, which is a package for building LEGO bricks in R. I had never tried the package, but I am sure, it´s quite fun as well. Give it a go!
